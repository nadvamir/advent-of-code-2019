...
size_t count_passwords(size_t so_far, size_t bottom, size_t up, size_t deleter) {
    if (so_far >= bottom && so_far <= up && is_valid(so_far)) return 1;
    ...
    for (; i < 10; ++i) {
        answer += count_passwords(so_far * 10 + i, bottom, up, deleter / 10);
    }
    return answer;
}
...
void run() {
    ...
    auto answer = count_passwords(0, bottom, up, 1000000);
