...
int calc_path(const auto& l, const auto p) {
    if (l.a.x == l.b.x) {
        if (l.rotated) return l.b.y - p.y;
        else return p.y - l.a.y;
    }
    if (l.a.y == l.b.y) {
        if (l.rotated) return l.b.x - p.x;
        else return p.x - l.a.x;
    }
    return 1 << 20;
}
...
size_t crossing_point(const std::string& w1, const std::string& w2) {
    ...
    for (const auto& l1 : w1_lines) {
        ...
        for (const auto& l2 : w2_lines) {
            auto x = l1.cross(l2);
            if (x.distance() != 0) {
                int new_p = w1d + w2d + calc_path(l1, x) + calc_path(l2, x);
                if (new_p < res) res = new_p;
            }
            w2d += l2.length();
        }
        w1d += l1.length();
...
void test() {
    ...
    TEST_EQ(crossing_point("R8,U5,L5,D3", "U7,R6,D4,L4"), 30);
    TEST_EQ(crossing_point("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"), 610);
    TEST_EQ(crossing_point("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"), 410);
}
